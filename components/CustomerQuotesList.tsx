...
type TabCounts = { waiting: number; offers: number; total: number };
...
export default function CustomerQuotesList() {
  ...
  const tab = (searchParams.get("tab") === "waiting" ? "waiting" : "offers") as CustomerQuoteTab;
  const [searchInput, setSearchInput] = useState(searchParams.get("q") ?? "");
  const [quotes, setQuotes] = useState<QuoteItem[]>([]);
  const [total, setTotal] = useState(0);
  const [tabCounts, setTabCounts] = useState<TabCounts>({ waiting: 0, offers: 0, total: 0 });
  ...
  const summaryHint = useMemo(() => {
    if (tab === "offers") return "Usta teklifi gelen veya anlaşma sürecindeki talepler";
          ...
          >
            ...
            {tabCounts.offers > 0 && (
              <span className="ml-1.5 rounded-full bg-white/20 px-2 py-0.5 text-xs">
                {tabCounts.offers.toLocaleString("tr-TR")}
              </span>
            )}
          </button>
          <button
          ...
          >
            ...
            {tabCounts.waiting > 0 && (
              <span className="ml-1.5 rounded-full bg-amber-100 px-2 py-0.5 text-xs text-amber-800">
                {tabCounts.waiting.toLocaleString("tr-TR")}
              </span>
            )}
          </button>
        </div>
